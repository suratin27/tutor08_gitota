...
uint8_t slaveIp[] = { 192, 168, 1, 10 };      //- Slave IP Address
...
uint16_t holdingData[] ={0,0,0,0,0};  //- ค่า Holding registor  
...
void loop(){
  ...
  if (slave.connected()) {
    if (millis() - lastSentTime > 500) {
      lastSentTime = millis();
      master.readInputRegisters(slave,31,0,5,&holdingData[5]);    //- ขอให้สังเกตค่านี้ 5 ทั้งสองตำแหน่งให้ดีครับ ในการใช้งานตัวเลขทั้งสองตัวนี้ต้องเท่ากัน
      master.readDiscreteInputs(slave,31,0,25,&coilData[25]);     //- ขอให้สังเกตค่านี้ 25 ทั้งสองตำแหน่งให้ดีครับ ในการใช้งานตัวเลขทั้งสองตัวนี้ต้องเท่ากัน
    }
  }  
}